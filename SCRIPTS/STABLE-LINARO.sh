...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	export curdate=`date "+%m-%d-%Y"`
        cp ~/Compile.log ~/android/Logs/Completed-STABLE-4.4-LINARO-$curdate.log
        rm ~/Compile.log

	cd $PACKAGEDIR
	rm ../STABLE-Android-4.4-LINARO-Cl3Kener-Nightly*.zip\
	rm -R .fr-7q5stU
	zip -r ../STABLE-Android-4.4-LINARO-Cl3Kener-Nightly-$curdate.zip .
	...
	export curdate=`date "+%m-%d-%Y"`
	cp ~/Compile.log ~/android/Logs/Failed-STABLE-4.4-LINARO-$curdate.log
fi;
